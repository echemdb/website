...
from .make_pages import cv_data, get_page_links
...
def get_table(filename):
    if filename.split(".")[-1] == "csv":
        ...
        return mdFile.new_table(
            columns, rows, text, text_align="center", marker=""
        )  # text is flattened array
...
def make_span(elementname, elecolor):
    return (
        '<span class="a" style="background-color:{}"> '.format(elecolor)
        + make_element_link(elementname)
        + " </span>"
    )
