...
from echemdb.data.legacy.build_data import (DISPLAYED_INFOS, ELEMENTS_DATA,
                                            TARGET_FOLDERS, TEMPLATE_FOLDERS,
                                            get_plotly_plot)
...
def get_echemdb_id_file(echemdb_id):
    target = copy.deepcopy(TARGET_FOLDERS["echemdb_id"]).replace(
        "tobesubstituted", echemdb_id
    )
    targetfile = target
...
def get_element_file(elementname):
    target = copy.deepcopy(TARGET_FOLDERS["elements"]).replace(
        "tobesubstituted", elementname
    )
    targetfile = target
    return targetfile.split(".md")[0]
...
def get_element_surface_file(elementname, surfacename):
    target = copy.deepcopy(TARGET_FOLDERS["element_surface"]).replace(
        "tobesubstituted", f"{elementname}-{surfacename}"
    )
    targetfile = target
    return targetfile.split(".md")[0]
...
def get_page_links(property_propertyvals_dict):
    if len(property_propertyvals_dict.keys()) == 2:
        elementname, surfacename = (
            property_propertyvals_dict["elementname"],
            property_propertyvals_dict["surfacename"],
        )
...
def create_nice_table_overview(df):
    ...
    dfc[
        "link"
    ] = "."  # dfc['path'].apply(lambda path: f'[:material-file-download:]({path})')
    dfc[
        "echemdb-id"
    ] = "."  # dfc['echemdb-id'].apply(lambda echemdb_id: f'[{echemdb_id}](../../../{get_echemdb_id_file(echemdb_id)})')
    # reference????
    return dfc
...
def create_element_pages(elementname):
    ...
    with mkdocs_gen_files.open(
        TARGET_FOLDERS["elements"].replace("tobesubstituted", elementname), "wb"
    ) as f:
...
def create_element_surface_pages(elementname, surfacename):
    ...
    with mkdocs_gen_files.open(
        TARGET_FOLDERS["elements"].replace(
            "tobesubstituted", f"{elementname}-{surfacename}"
        ),
        "wb",
    ) as f:
...
def get_element_page_contents(elementname):
    ...
    for t in [tupled for tupled in grouped_cv_data.groups if tupled[0] == elementname]:
        ...
        ele_list_md_str = get_filtered_tables(
            t[0], surface=t[1]
        )  # here we could leave away surface, filter differently
