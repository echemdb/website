...
TEMPLATE_FOLDERS = {
    "elements": "./templates/pages/legacy/element.md",
...
ELEMENTS_DATA = {"elements_data": "./data/elements.csv"}
    ...
    try:
        ...
        if zipfile.is_zipfile(the_zip):
            with zipfile.ZipFile(the_zip, "r") as z:
                _validate_zip(z)
                descriptor_path = [
                    f for f in z.namelist() if f.endswith("datapackage.json")
                ][0]
...
def make_plotly_figure_from_dataframes(alldfs):
    ...
    for (descr, df, meta) in alldfs:
        ...
        fig.add_trace(
            go.Scatter(x=df["potential"], y=df["current"], mode="lines", name=descr)
        )
        fig.update_layout(
    ...
    fig.update_yaxes(
        mirror="allticks", ticks="inside", title={"text": "current (mA)"}
    )  # range=[0., 400],
